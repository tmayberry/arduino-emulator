...
function newComponentId(type: "potentiometer" | "sensor" | "toggle-switch"): string {
  const unique = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${fallbackId++}`;
  return `custom-${type}-${unique}`;
}

export function BoardSetupDialog({ config, onApply, onClose }: BoardSetupDialogProps) {
  ...
  const usedPins = new Set(configurable.map((component) => normalizePin(component.pin)));
  const hasDuplicatePins = usedPins.size !== configurable.length;
  const hasInvalidLabel = configurable.some(
    (component) => !component.label.trim() || component.label.trim().length > 50,
  );
  ...
  const freeDigitalPins = DIGITAL_DEVICE_PINS.filter((pin) => !usedPins.has(`D${pin}`));
  const freeAnalogPins = ANALOG_DEVICE_PINS.filter((pin) => !usedPins.has(pin));

  const counts = useMemo(() => ({
    potentiometers: configurable.filter((item) => item.type === "potentiometer").length,
    sensors: configurable.filter((item) => item.type === "sensor").length,
    switches: configurable.filter((item) => item.type === "toggle-switch").length,
  }), [configurable]);

  const update = (
    id: string,
    changes: Partial<LedComponentConfig | PotentiometerComponentConfig | SensorComponentConfig | ToggleSwitchComponentConfig>,
  ) => {
    setDraft((current) => ({
      ...current,
      components: current.components.map((component) =>
        component.id === id ? { ...component, ...changes } as typeof component : component,
      ),
    }));
  };
  ...
  const apply = () => {
    ...
    if (!green || typeof green.pin !== "number" || hasDuplicatePins || hasInvalidLabel || hasInvalidSensor) return;
    const nextConfig: HardwareConfig = {
      ...draft,
      testLedPin: green.pin,
      components: draft.components.map((component) => component.type === "sensor"
        ? {
            ...component,
            label: component.label.trim(),
            units: component.units.trim(),
            defaultValue: component.rangeStart + (component.rangeEnd - component.rangeStart) / 2,
          }
        : { ...component, label: component.label.trim() }),
    };
    nextConfig.name = isDefaultHardware(nextConfig) ? "Default Hardware" : "Custom Hardware";
    onApply(nextConfig);
  };
      ...
      <section className="setup-dialog" role="dialog" aria-modal="true" aria-labelledby="setup-title">
        <header className="setup-dialog-header">
          <div>
            ...
            <h2 id="setup-title"><Settings size={19} aria-hidden="true" /> Board setup</h2>
          </div>
          <button className="icon-button" type="button" onClick={onClose} aria-label="Close board setup" autoFocus>
        ...
        <div className="setup-device-list">
          {configurable.map((component) => {
            const isBuiltIn = component.type === "led" && component.placement === "board";
            ...
            return (
              <div className="setup-device-row" key={component.id}>
                <span className="setup-device-type">
                  {isBuiltIn ? "Built-in LED" : component.type === "led" ? "External LED" : component.type === "potentiometer" ? "Potentiometer" : component.type === "sensor" ? "Sensor" : "Switch"}
                </span>
                <label>
                  ...
                  <input
                    ...
                    maxLength={50}
                    disabled={isBuiltIn}
                    onChange={(event) => update(component.id, { label: event.currentTarget.value })}
                  />
                </label>
                <label>
                  ...
                  <select
                    ...
                    disabled={isBuiltIn}
                    onChange={(event) => update(component.id, {
                      pin: component.type === "potentiometer" || component.type === "sensor"
                  ...
                  >
                    {pinOptions.map((pin) => {
                      ...
                      const occupied = usedPins.has(normalized) && normalized !== normalizePin(component.pin);
                      return <option key={pin} value={pin} disabled={occupied}>{typeof pin === "number" ? `D${pin}` : pin}</option>;
                    })}
                  </select>
                </label>
                {component.origin === "custom" ? (
                  <button className="remove-device-button" type="button" onClick={() => remove(component.id)} aria-label={`Remove ${component.label}`}>
                  ...
                  <div className="sensor-range-fields">
                    <label>
                      ...
                      <input
                        ...
                        type="number"
                        step="any"
                        value={Number.isFinite(component.rangeStart) ? component.rangeStart : ""}
                        onChange={(event) => update(component.id, {
                          rangeStart: event.currentTarget.value === "" ? Number.NaN : Number(event.currentTarget.value),
                        })}
                      />
                    </label>
                    <label>
                      ...
                      <input
                        ...
                        type="number"
                        step="any"
                        value={Number.isFinite(component.rangeEnd) ? component.rangeEnd : ""}
                        onChange={(event) => update(component.id, {
                          rangeEnd: event.currentTarget.value === "" ? Number.NaN : Number(event.currentTarget.value),
                        })}
                      />
                    </label>
                    <label>
                      ...
                      <input
                        ...
                        onChange={(event) => update(component.id, { units: event.currentTarget.value })}
                      />
                    </label>
                  </div>
                )}
        ...
        <div className="setup-add-controls">
          <button type="button" onClick={addSwitch} disabled={freeDigitalPins.length === 0}>
            <Plus size={16} aria-hidden="true" /> Add switch
          </button>
          <button type="button" onClick={addPotentiometer} disabled={freeAnalogPins.length === 0}>
            <Plus size={16} aria-hidden="true" /> Add potentiometer
          </button>
          <button type="button" onClick={addSensor} disabled={freeAnalogPins.length === 0}>
        ...
        <footer className="setup-dialog-actions">
          ...
          <button className="primary-button" type="button" onClick={apply} disabled={hasInvalidLabel || hasDuplicatePins || hasInvalidSensor}>Apply setup</button>
        </footer>
      </section>
    </div>
  );
