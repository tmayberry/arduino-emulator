...
function ledBrightness(
  ...
  if (!output) return 0;
  const level =
    output.kind === "pwm" ? output.value / 255 : output.value === 0 ? 0 : 1;
...
export function HardwareView({
  ...
  const sensors = config.components.filter(
    (component): component is SensorComponentConfig =>
  ...
  const inputSummary = [
    ...pots.map(
      (pot) =>
        `${pot.pin} ${Number(componentInputs[pot.id] ?? pot.defaultValue)}`,
    ),
    ...sensors.map((sensor) => {
    ...
    ...toggles.map(
      (toggle) =>
        `D${toggle.pin} ${componentInputs[toggle.id] ? "HIGH" : "LOW"}`,
    ),
  ].join(" · ");
  ...
  return (
    <section
      className="workspace-panel hardware-panel"
    ...
    >
        ...
        <div className="breadboard">
          ...
          <div className="board-module">
            ...
            <div className="nano-board">
              <div className="nano-pins left-pins">
              ...
              <div className="nano-pins right-pins">
                {Array.from({ length: 9 }, (_, i) => (
                  <i key={i} />
                ))}
              </div>
          ...
          <div className="led-bank">
            {breadboardLeds.map((led) => (
              <Led
                key={led.id}
                component={led}
                brightness={ledBrightness(led, pinOutputs)}
      ...
      >
        ...
        >
          ...
          <span>
            Inputs<small>{inputSummary || "No inputs"}</small>
          </span>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <span>
            Motion<small>{motionSummary}</small>
          </span>
        </button>
      </div>
      ...
      {activeTab === "inputs" && (
        <div
          className="hardware-tab-panel input-grid"
          role="tabpanel"
          aria-label="Inputs"
        >
              ...
              {column.map((component) =>
                component.type === "potentiometer" ? (
                  <Potentiometer
                    ...
                    component={component}
                    value={Number(
                      componentInputs[component.id] ?? component.defaultValue,
                    )}
      ...
      {activeTab === "motion" && (
        <div
          className="hardware-tab-panel motion-panel"
          role="tabpanel"
          aria-label="Motion"
        >
          <AccelerometerPanel
