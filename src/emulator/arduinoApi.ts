...
function installArduinoString(rt: JscppRuntime): InstalledArduinoString {
  ...
  const stringType = rt.newClass("String", [
    {
      name: "__data",
      type: charPointer,
      initialize: (runtime: JscppRuntime) =>
        runtime.makeCharArrayFromString(""),
    },
  ]);
  ...
  const textOf = (value: RuntimeValue): string => {
    ...
    if (isType(value.t, rt.boolTypeLiteral))
      return Number(value.v) === 0 ? "0" : "1";
    if (
  ...
  const requireLeftValue = (value: RuntimeValue): void => {
    if (!value.left)
      throw new Error("String assignment target is not a variable");
  };
  ...
  rt.regFunc(
    (_runtime, value) => rt.makeCharArrayFromString(textOfString(value)),
    stringType,
    "c_str",
    [],
    charPointer,
...
export function createArduinoInclude(
  ...
  return {
    load(rt) {
      ...
      rt.regFunc(
        (runtime) => runtime.val(long, engine.millis()),
        "global",
        "millis",
        [],
        long,
      ...
      rt.regFunc(
        (runtime) => runtime.val(long, engine.micros()),
        "global",
        "micros",
        [],
        long,
      ...
      rt.regFunc(
        (runtime, _this, value, fromLow, fromHigh, toLow, toHigh) =>
          runtime.val(
            long,
            engine.map(
              numberValue(value),
      ...
      const registerSerialOverload = (
        ...
        rt.regFunc(
          (_runtime, _this, value) => writeSerial(format(value), false),
          serialType,
          "print",
          [argumentType],
          int,
        ...
        rt.regFunc(
          (_runtime, _this, value) => writeSerial(format(value), true),
          serialType,
          "println",
          [argumentType],
          int,
      ...
      registerSerialOverload(rt.unsignedintTypeLiteral, (value) =>
        String(numberValue(value)),
      );
      registerSerialOverload(long, (value) => String(numberValue(value)));
      registerSerialOverload(rt.floatTypeLiteral, (value) =>
        numberValue(value).toFixed(2),
      );
      registerSerialOverload(rt.doubleTypeLiteral, (value) =>
        numberValue(value).toFixed(2),
      );
      ...
      registerSerialOverload(
        rt.normalPointerType(rt.charTypeLiteral),
        (value) => rt.getStringFromCharArray(value),
      );
