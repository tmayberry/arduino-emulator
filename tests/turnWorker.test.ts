    ...
    getByName(sessionId: string) {
      return {
        ...
        async storeOffer(offerToken: string) {
          ...
          if (!session || session.expiresAt < Date.now() || session.answerToken)
            return false;
        ...
        async storeAnswer(answerToken: string) {
          const session = sessions.get(sessionId);
          if (!session || session.expiresAt < Date.now() || !session.offerToken)
            return false;
          if (session.answerToken && session.answerToken !== answerToken)
            return false;
...
function request(
  ...
  origin = "https://tmayberry.github.io",
) {
...
describe("TURN credential and pairing Worker", () => {
  it("completes a role-authorized pairing exchange through a session mailbox", async () => {
    ...
    expect(startResponse.headers.get("Access-Control-Allow-Origin")).toBe(
      "https://tmayberry.github.io",
    );
    ...
    const offer = await worker.fetch(
      request(`${base}/offer`, {
    ...
    const join = await worker.fetch(
      request(`${base}/join`, { grant: start.phoneGrant }),
      env,
    );
    ...
    const pending = await worker.fetch(
      request(`${base}/poll`, { grant: start.desktopGrant }),
      env,
    );
    ...
    expect(JSON.stringify(joined.iceServers)).not.toContain(
      "cloudflare.com:53",
    );
  ...
  it("rejects altered grants and grants used for the wrong role", async () => {
    ...
    const altered = await worker.fetch(
      request(`${base}/join`, { grant: `${start.phoneGrant}x` }),
      env,
    );
    const wrongRole = await worker.fetch(
      request(`${base}/poll`, { grant: start.phoneGrant }),
      env,
    );
  ...
  it("rejects incorrect course codes and unapproved browser origins", async () => {
    ...
    const badCode = await worker.fetch(
      request("/v2/pairing/start", { accessCode: "wrong" }),
      env,
    );
